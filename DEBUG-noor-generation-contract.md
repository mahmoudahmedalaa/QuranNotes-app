...
12. 2026-09-01 — Final-SHA transcript verification exposed a summary-only ownership mismatch: the planner's existing broad-operation classifier routes learning/lesson synthesis to `entity_summary`, while the qualifier reparses the same operation as a constrained lexical/teaching relation and requires literal relation tokens beyond entity-wide coverage.
13. 2026-09-01 — Adversarial review confirmed the initial summary-learning correction was overbroad: accepting every `lexical:*` relation allows an unrelated requested attribute or procedure to be coerced to teaching merely because the question also contains a broad-operation cue.
