...
The evaluator is runnable without changing the callable: provide an external JSON file containing sanitized traces and optional expectations, then run `cd functions && npm run noor:eval -- --input=/absolute/path/to/sanitized-traces.json`. The input path is deliberately outside production code; adding a question, paraphrase, language variant, or adversarial row changes evaluation data only. Lexical availability is reported separately from lexical hit count so a vector-only fallback cannot be mistaken for a healthy hybrid run.
