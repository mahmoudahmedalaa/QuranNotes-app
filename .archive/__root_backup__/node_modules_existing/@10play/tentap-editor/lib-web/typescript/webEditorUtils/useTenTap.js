...
export const useTenTap = (options) => {
    ...
    const editor = useEditor(Object.assign({ content, onCreate: () => sendMessage({
        ...
        }, onSelectionUpdate: (onUpdate) => sendStateUpdate(onUpdate.editor), onTransaction: (onUpdate) => sendStateUpdate(onUpdate.editor), editable: window.editable, shouldRerenderOnTransaction: true }, tiptapOptionsWithExtensions));
    useEffect(() => {
        if (!editor)
            return;
