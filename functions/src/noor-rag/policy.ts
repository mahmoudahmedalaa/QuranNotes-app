...
const EXPLICIT_PERSONALIZED_RULING_COMMAND = /\b(?:issue|give|provide|decide|determine)\b.{0,60}\b(?:personal|individual|tailored)\b.{0,30}\b(?:fatwa|ruling)\b|(?:أعطني|أصدر|حدد).{0,40}(?:فتوى شخصية|حكم شرعي شخصي)/i;
...
export function classifyPolicy(content: string): NoorPolicyCategory {
    ...
    if (EXPLICIT_PERSONALIZED_RULING_COMMAND.test(content)) {
